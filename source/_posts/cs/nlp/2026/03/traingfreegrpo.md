...
腾讯优图提出的Training-Free Group Relative Policy Optimization（Training-Free GRPO）方法，仅用8-18美元和100条样本，就让671B参数的DeepSeek-V3.1-Terminus在AIME数学竞赛任务上获得显著提升，甚至超过花费数万美元微调的32B模型。  
...
为了理解Training-Free GRPO的创新，我们先回顾下GRPO。  
...
对于问题 $q$，从当前策略采样 $G=5$ 条轨迹，每条轨迹 $o_i$ 包含推理过程、工具调用和最终答案。  
...
多维度奖励扩展：虽然论文实验主要使用二元奖励，但框架支持多种奖励的加权合并：  
...
这里 $M$ 是LLM（保持参数冻结），$p_{\text{summary}}$ 是摘要提示词。关键在于：摘要过程中会通过`<evaluation>`标签隐式注入奖励信号（"correct"或"wrong"），让模型知道这条轨迹最终是成功还是失败。
...
Training-Free GRPO的本质是通过精心设计的Prompt链，将RL的信用分配机制转化为自然语言操作。  
...
这两个阶段也可以合并为一个prompt。论文实现中采用的是合并方式（end-to-end）：  
...
这是消融实验中的关键配置，指奖励信号不依赖标准答案，而是通过组内自举（bootstrapping）获得：
...
2. **快速迭代场景**：策略需要频繁更新，无法承受每次训练数小时的延迟  
...
3. **提示工程敏感**：$p_{\text{summary}}$ 和 $p_{\text{extract}}$ 的设计影响效果，需要一定调优  
