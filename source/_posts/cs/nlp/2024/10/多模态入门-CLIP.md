...
- 以英文维基百科中出现至少 100 次的所有单词为基础构建词集，并增加了双词组合和所有 WordNet 同义词，最终构建了约50w词的词库，保证覆盖的概念够多  
- 爬取网上的数据，使（图像，文本）数据对中的文本包含词集中至少一个词，也就是这些图都包含重要概念的内容  
- 图文对中的文本不是固定的单词label，而是自由形式的：可以来源于网页标题、图片描述甚至用户评论，因此内容也是多种多样的  
- 为了尽可能覆盖广泛的视觉概念，对结果进行平衡，每个概念最多包括 20,000 个（图像，文本）对；比如「猫」的图片就非常多，但是不能让高频概念的图片完全主导数据集，而是各个概念平衡  
- 使用了规则过滤了各种噪声，比如广告、版权生命，相机的拍照参数等  
...
在clip的训练框架中，text encoder和image encoder的地位是对称的。看上面代码中的loss计算，取的是二者的平均。  
...
CLIP另一个强项就是可以做zero-shot predictor。比如我们想要知道让预训练模型对一张图片的类别进行预测，可以把所有可能的类别填进一个prompt里：“A photo of {object}”，我们可以给定一个object的列表，然后让text encoder给出列表各个object的prompt representation，并计算不同类别下的text representation和image representation的相似度，取最高的那个就是预测结果了：  
