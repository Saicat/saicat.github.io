...
这一阶段引入了高质量和细粒度的 fine-grained VL annotation data，包含交错的图文数据，时候用的分辨率也更大（224×224 -> 448×448）。所用的任务数据如下表：  
...
- 用于对齐部分也比较小，切一般是随机初始化，因此对齐效果不够好  
...
QLLaMA集成第一阶段训练得到的LLaMA-7B参数，保持InternViT-6B和QLLaMA的参数冻结，使用进一步过滤的后剩下的1B高质量图文数据进行训练。  
...
上面的实验还能看待，增加文本数据后，多模态能力不会出现明显的损失。结合一系列观察，选择了7（语言）:3（多模态）的数据比例。  
